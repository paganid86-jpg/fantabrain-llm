...
class AssemblyError(ValueError):
    """Raised when Dataset v3 cannot be assembled safely."""
...
def validate_p3_record(
    *,
    ...
    required_tags = {"v3", "train", str(mode), str(task), focus_tag}
...
def assemble_dataset(base_path: Path, manifest_path: Path, output_path: Path) -> dict[str, object]:
    ...
    required_source = str(quality_gates.get("source", "v3_manual"))
    forbidden_prompts = load_pagella_prompts(
        eval_path,
        required=bool(quality_gates.get("forbid_eval_prompt_leakage", False)),
    )

    p3_records: list[dict[str, object]] = []
    for block in manifest.get("p3_blocks", []):
        ...
        p3_records.extend(records)

    expected_total = int(manifest["p3_examples"])
    if len(p3_records) != expected_total:
        raise AssemblyError(f"expected {expected_total} P3 examples, got {len(p3_records)}")

    expected_by_mode = manifest["p3_balance"]["by_mode"]
    p3_by_mode = count_modes(p3_records, expected_by_mode)
    if p3_by_mode != expected_by_mode:
        raise AssemblyError(f"expected P3 mode split {expected_by_mode}, got {p3_by_mode}")

    output_records = [*base_records, *p3_records]
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Assemble and validate Dataset v3.")
    parser.add_argument("--base", required=True, help="Final Dataset v2 JSONL path.")
    parser.add_argument("--manifest", required=True, help="Dataset v3 manifest path.")
    parser.add_argument("--output", required=True, help="Output Dataset v3 train JSONL path.")
